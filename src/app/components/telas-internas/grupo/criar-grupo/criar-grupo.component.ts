  ...
  abrirModal() {
    Swal.fire({
      ...
      preConfirm: () => {
        ...
        const alunoAdminId = (
          document.getElementById('aluno-admin-id') as HTMLInputElement
        ).value;
        const alunosIdsString = (
          document.getElementById('alunos-ids') as HTMLInputElement
        ).value;
        ...
        const alunosIds = alunosIdsString
          .split(',')
      ...
      if (result.isConfirmed && result.value) {
        this.grupoService.criarGrupo(result.value as GrupoDto).subscribe({
          next: (response: string) => {
            // Tipagem explícita para o 'response'
